...
>({
  ...
  const [loadState, setLoadState] = useState<LoadState<T>>({
    state: shouldLoad ? State.loading : State.init,
  });
  const [loadTimes, setLoadTimes] = useState(0);

  const loading = loadState.state === State.loading;
  ...
  const load = useCallback(
    async (reload = false) => {
      const canFetch =
        stateRef.current === State.init ||
        (reload && stateRef.current !== State.loading);
      if (!canFetch) return;
  ...
  useEffect(() => {
    ...
    if (stateRef.current === State.init) {
